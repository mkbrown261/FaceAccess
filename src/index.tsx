...
import businessAuth from './routes/business-auth'
import businessApi from './routes/business-api'
...
// ── Security middleware ──────────────────────────────
// CORS: the dashboard is same-origin. Reflect the request origin so custom
// domains / preview deployments work; auth is bearer-token based (no cookies),
// so reflecting origin does not create a CSRF surface.
app.use('/api/*', cors({
...
// ── Business product (multi-tenant, org-scoped, auth-required) ──
app.route('/api/auth/business', businessAuth)
...
app.post('/api/home/users/:id/face', async (c) => {
  ...
  if (typeof embedding === 'string' && embedding.length > 20 && embedding.length < 65536) {
    ...
    return bad(c, 'A face embedding is required')
  }
...
app.post('/api/home/guests/:id/face', async (c) => {
  ...
  if (!embedding || !Array.isArray(embedding) || !embedding.every((v: unknown) => typeof v === 'number' && isFinite(v))) {
    return bad(c, 'A face embedding is required')
  }
  const emb = embedding
...
<div id="auth-wall" style="position:fixed;inset:0;z-index:9999;background:#0a0a14;display:flex;align-items:center;justify-content:center;padding:16px;">
  <div style="width:100%;max-width:440px;">
    ...
    <div id="biz-form-login">
      <div style="background:#13131f;border:1px solid #1e1e30;border-radius:16px;padding:24px;">
        ...
        <p style="text-align:center;margin-top:14px;font-size:12px;color:rgba(255,255,255,0.3)">
          New to FaceAccess? <a onclick="bizShowTab('register')" style="color:#a5b4fc;cursor:pointer">Create your organization</a>
        </p>
      </div>
    </div>
    ...
    <div id="biz-form-register" style="display:none">
      <div style="background:#13131f;border:1px solid #1e1e30;border-radius:16px;padding:24px;">
        <div id="biz-invite-banner" style="display:none;background:rgba(16,185,129,.08);border:1px solid rgba(16,185,129,.25);border-radius:10px;padding:12px 14px;margin-bottom:16px;font-size:12.5px;color:#a7f3d0;line-height:1.5"></div>
        ...
        <div id="biz-reg-org-block">
          <div style="margin-bottom:14px">
            <label style="font-size:11px;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:6px">Organization Name *</label>
            <input id="biz-reg-org" class="auth-input" placeholder="Acme Corp" maxlength="120">
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
            <div>
              <label style="font-size:11px;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:6px">Company Size</label>
              <select id="biz-reg-size" class="auth-input">
            ...
            <div>
              <label style="font-size:11px;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:6px">Industry</label>
              <input id="biz-reg-industry" class="auth-input" placeholder="e.g. Manufacturing" maxlength="80">
            </div>
          </div>
        </div>
        <div style="margin-bottom:16px">
        ...
        <button id="biz-reg-btn" onclick="bizDoRegister()"
          ...
          <i class="fas fa-user-plus mr-2"></i><span id="biz-reg-btn-label">Create Organization</span>
        </button>
        <p style="text-align:center;margin-top:12px;font-size:11px;color:rgba(255,255,255,0.3)">You'll be the admin of your organization. Invite your team from Settings &rarr; Team.</p>
      </div>
    </div>
  </div>
</div><!-- end auth-wall -->
...
<aside class="sidebar w-64 flex-shrink-0 flex flex-col h-screen sticky top-0">
  <div class="p-5 border-b border-gray-800">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-xs text-gray-500 truncate max-w-[150px]" id="biz-org-name">Access Control</p>
      </div>
    </div>
  </div>
  ...
  <nav class="p-3 flex-1 space-y-1 overflow-y-auto">
    ...
    <a onclick="showPage('approvals')" class="nav-item" id="nav-approvals">
      <i class="fas fa-user-check w-5 text-center"></i> Approvals
      <span id="approvals-badge" class="ml-auto hidden text-[10px] font-bold bg-amber-500 text-black rounded-full px-1.5 py-0.5">0</span>
    </a>
    ...
    <a onclick="showPage('team')" class="nav-item" id="nav-team">
      <i class="fas fa-user-friends w-5 text-center"></i> Team
    </a>
  ...
  <div class="p-4 border-t border-gray-800">
    <div class="flex items-center gap-3">
      ...
      <div class="text-sm min-w-0">
        <div id="biz-user-name" class="font-medium text-white truncate">—</div>
        <div id="biz-user-email" class="text-xs text-gray-500 truncate">—</div>
      </div>
...
<main class="flex-1 overflow-auto">
  ...
  <header class="sticky top-0 z-10 bg-gray-950/80 backdrop-blur border-b border-gray-800 px-6 py-3 flex items-center gap-4">
    ...
    <div class="flex items-center gap-3">
      ...
      <div id="plan-badge" class="hidden text-xs px-2.5 py-1 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-300"></div>
      <div class="flex items-center gap-1.5 text-xs text-green-400" id="engine-status">
        <div class="w-1.5 h-1.5 rounded-full bg-gray-500" id="engine-dot"></div> <span id="engine-label">Loading face engine…</span>
      </div>
    </div>
  </header>
  ...
  <div id="page-approvals" class="page p-6 hidden">
  </div>

  <div id="page-team" class="page p-6 hidden">
  </div>
